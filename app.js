...
async function loadData(force=false){
  try{
    ...
    const [pRes,hRes,csvRes]=await Promise.all([
      fetch(DATA_URL+q), fetch(HISTORY_URL+q), fetch(PREDICTIONS_URL+q)
    ]);
...
function renderVenues(){
  ...
  const hasMap={};
  ALL_PROGRAMS.forEach(p=>{ if(p.race_date===targetDate) hasMap[p.race_stadium_number]=true; });
...
function renderRaceDetail(venueId,raceNo){
  ...
  const players=(prog.boats||[]).map(b=>{
    ...
    return {lane:b.racer_boat_number,name:b.racer_name,klass:b.racer_class||"-",st,local,motor,course,
      rawScore:(1/(st||0.3))*(motor/100||0.3)*(local/100||0.3)*(course/100||0.3)};
  }).sort((a,b)=>a.lane-b.lane);

  players.forEach(p=>{
    const tr=document.createElement("tr");
    tr.innerHTML=`<td>${p.lane}</td><td>${p.klass} ${p.name} ST:${p.st||"-"}</td><td>-</td><td>${p.local||"-"}%</td><td>${p.motor||"-"}%</td><td>${p.course||"-"}%</td><td>-</td>`;
    entryTableBody.appendChild(tr);
  });

  // AI順位予測 (predictions.csv を優先)
  rankingTableBody.innerHTML="";
  const preds=PREDICTIONS.filter(r=>+r.venue_id===venueId && +r.race_no===raceNo && r.date===targetDate);
  if(preds.length){
  ...
  aiSubBody.innerHTML="";
  preds.filter(r=>r.type==="main").forEach(r=>{
    aiMainBody.innerHTML+=`<tr><td>${r.combination}</td><td>${r.prob||"--"}%</td></tr>`;
  });
  preds.filter(r=>r.type==="sub").forEach(r=>{
  ...
  players.forEach(p=>{
    const tr=document.createElement("tr");
    tr.innerHTML=`<td>${p.lane}</td><td>${p.name} の展開次第</td>`;
