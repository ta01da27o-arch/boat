...
function renderVenues(){
  ...
  ALL_PROGRAMS.forEach(p => {
    ...
    const stadium = p.race_stadium_number || p.jcd;
  ...
  VENUE_NAMES.forEach((name, idx) => {
    ...
    card.innerHTML = `<div class="v-name">${name}</div><div class="v-status">${has?"開催中":"ー"}</div><div class="v-rate">${hitText}</div>`;
...
function renderRaces(venueId) {
  ...
  racesGrid.innerHTML = "";
  const targetDate = (CURRENT_MODE === "today") ? getIsoDate(new Date()) : getIsoDate(new Date(Date.now() - 86400000));
  const progs = ALL_PROGRAMS.filter(p => (p.race_date || p.date) === targetDate && (p.race_stadium_number || p.jcd) === venueId);
...
async function renderRaceDetail(venueId, raceNo) {
  showScreen("race");
  const targetDate = (CURRENT_MODE === "today") ? getIsoDate(new Date()) : getIsoDate(new Date(Date.now() - 86400000));
  const prog = ALL_PROGRAMS.find(p => (p.race_date||p.date)===targetDate && (p.race_stadium_number||p.jcd)===venueId && (+p.race_number||+p.race_no)===raceNo);
  if (!prog) return;
  ...
  const players = boats.map(b => ({
    ...
    name: b.racer_name || "-",
    klass: formatKlass(b),
    st: safeNum(b.racer_average_start_timing),
    local: formatRateRaw(b.local_win_rate),
    motor: formatRateRaw(b.motor_win_rate),
    course: formatRateRaw(b.boat_win_rate)
  }));
  ...
  players.forEach(p => {
    ...
    tr.innerHTML = `<td>${p.lane}</td><td>${p.klass}</td><td>${p.name}</td>
                    <td>${p.local}%</td><td>${p.motor}%</td><td>${p.course}%</td>`;
    entryTableBody.appendChild(tr);
  });

  // === AI買い目 ===
  aiMainBody.innerHTML = "";
  ai.main.forEach(r => aiMainBody.innerHTML += `<tr><td>${r.combo}</td><td>${r.prob}%</td></tr>`);
  aiSubBody.innerHTML = "";
  ai.sub.forEach(r => aiSubBody.innerHTML += `<tr><td>${r.combo}</td><td>${r.prob}%</td></tr>`);
  ...
  ai.ranks.forEach(r => rankingTableBody.innerHTML += `<tr><td>${r.rank}</td><td>${r.lane}</td><td>${r.name}</td><td>${r.score}</td></tr>`);
}
...
/* 起動 */
loadData();
window.addEventListener("error", ev => logStatus("ページエラー: " + ev.message));