...
backBtn?.addEventListener('click', ()=> {
  if(STATE.screen === 'races'){ STATE.screen='venues'; STATE.venueId = null; }
  else if(STATE.screen === 'race'){ STATE.screen='races'; STATE.raceNo = null; }
  render();
});
...
function showScreen(name){
  ...
  if(name === 'venues'){ headerBack.hidden = true; }
  else { headerBack.hidden = false; }
}
...
function renderVenues(){
  ...
  // use fixed order from data. If less than 24 provided, fill placeholders.
  const venues = STATE.data.venues.slice(0,24);
  for(let i=0;i<24;i++){
    const v = venues[i] || { id:`empty${i}`, name:'-', hasRacesToday:false, hitRate:'--' };
    const div = document.createElement('div'); div.className = 'venue';
    const hitText = (v.hitRate!=null && v.hitRate!=='--') ? `${v.hitRate}%` : '--%';
    const statusText = v.hasRacesToday ? '開催中' : '本日なし';
    div.innerHTML = `
      <div class="name">${v.name}</div>
      <div class="status">${statusText}</div>
      <div class="hit">${hitText}</div>
    `;
    const btn = document.createElement('button');
    btn.className = `venue-btn ${v.hasRacesToday? '' : 'disabled'}`;
    btn.textContent = v.hasRacesToday ? '開催中を確認' : '本日なし';
    if(v.hasRacesToday){
...
function renderRaces(){
  ...
  // get races: prefer top-level mapping, then venue.races
...
/* produce symbols based on values array (6 items) */
...
function renderRace(){
  ...
  // env pills
  const env = race.env || {};
  $('#envPills').innerHTML = `<span class="pill">発走 ${race.startTime||'-'}</span> <span class="pill">風 ${env.windDir||'-'}</span> <span class="pill">風速 ${env.windSpeed??'-'} m/s</span> <span class="pill">波高 ${env.wave??'-'} cm</span>`;

  const entries = race.entries || [];
  // ranking values (using first element or value)
  ...
  aiMain.forEach(item=>{
    const buy = (typeof item === 'string') ? item : (item.bet || item.buy || '-');
    const prob = (item.rate ?? item.probability) != null ? `${item.rate ?? item.probability}%` : '-';
    const tr = document.createElement('tr'); tr.innerHTML = `<td>${buy}</td><td class="mono">${prob}</td>`;
